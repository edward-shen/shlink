...
describe("copyLinkToClipboard", () => {
  ...
  const writeToOffscreenClipboardSpy = spyOn(
    offscreen_handler,
    "writeToOffscreenClipboard",
  );
  ...
  test("throws an exception if navigator clipboard doesn't exist", async () => {
    ...
    const naviClipboardSpy = spyOn(navigator.clipboard, "writeText");
    naviClipboardSpy.mockImplementationOnce(async () => {
      throw new Error("mock error");
    });

    expect(async () => {
      await clipboard.copyLinkToClipboard(dummyResponse);
    }).toThrow("Failed to copy to clipboard");
    mock.restore();
  });
});

describe("isChrome", async () => {
  function getChromeGlobalAndNavi({
    chrome,
    navi,
  }: {
    chrome: boolean;
    navi: boolean;
  }): [typeof chrome, Clipboard | undefined] {
  ...
  function testName(defined: boolean): string {
    return (defined ? "" : "un") + "defined";
  }
  ...
  for (const { useChrome, useNavi, expected } of table) {
    test(`${testName(useChrome)}/${testName(useNavi)} chrome global/navi clipboard returns ${expected}`, async () => {
      const [_, navi] = getChromeGlobalAndNavi({
        chrome: useChrome,
        navi: useNavi,
      });
