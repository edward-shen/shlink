...
interface ShlinkConfig extends Omit<DeepRequired<StorageAreaShlinkConfig>, "allowedProtocols"> {
...
type StorageAreaType = string | number | boolean | Array<StorageAreaType> | CreateOptions | ModifyOptions;
...
class ConfigManager {
    ...
    async #loadStorage(args?: any): Promise<StorageAreaShlinkConfig> {
        return await this.#storage.get(args) as unknown as StorageAreaShlinkConfig;
    }

    async #setStorage<T extends StorageAreaType>(key: keyof StorageAreaShlinkConfig, value: T) {
    ...
    async #getOrInit<
        Type extends StorageAreaType,
    >(fieldName: keyof StorageAreaShlinkConfig, defaultValue: Type): Promise<Type> {
    ...
    #getFactoryMapped<
        RawType extends StorageAreaType,
        MappedType,
    >(fieldName: keyof StorageAreaShlinkConfig, defaultValue: RawType, mapFn: (rawType: RawType) => MappedType): () => Promise<MappedType> {
        return () => this.#getOrInit(fieldName, defaultValue).then(mapFn);
    }

    #getFactory<K extends keyof StorageAreaShlinkConfig>(fieldName: K): () => Promise<DeepRequired<StorageAreaShlinkConfig>[K]> {
    ...
    #mapFactory<GetOut, SetIn>(
        ...
        return (f: (_: GetOut) => SetIn) => get().then(async (v) => f(v)).then(set);
    }

    #setFactoryMapped<
        RawType extends StorageAreaType,
        MappedType,
    >(fieldName: keyof StorageAreaShlinkConfig, mapFn: (rawType: MappedType) => RawType): (mapped: MappedType) => void {
        return (arg: MappedType) => this.#setStorage(fieldName, mapFn(arg));
    }

    #setFactory<
        RawType extends StorageAreaType,
    >(fieldName: keyof StorageAreaShlinkConfig): (mapped: RawType) => void {
    ...
    getButtonOption: () => Promise<ButtonOption> = this.#getFactory("shlinkButtonOption");
    getCreateOptions: () => Promise<Required<CreateOptions>> = this.#getFactory("createOptions");
    getModifyOptions: () => Promise<Required<ModifyOptions>> = this.#getFactory("modifyOptions");
    ...
    setButtonOption: (option: string) => void = this.#setFactory("shlinkButtonOption");
    setCreateOptions: (options: CreateOptions) => void = this.#setFactory("createOptions");
    setModifyOptions: (options: ModifyOptions) => void = this.#setFactory("modifyOptions");
    setAllowedProtocols: (procotols: Set<string>) => void = this.#setFactoryMapped(
        "allowedProtocols",
        (items) => new Array(...items),
    );

    mapCreateOptions = this.#mapFactory(this.getCreateOptions, this.setCreateOptions);
    mapModifyOptions = this.#mapFactory(this.getModifyOptions, this.setModifyOptions);
    mapAllowedProtocols = this.#mapFactory(this.getAllowedProtocols, this.setAllowedProtocols);
